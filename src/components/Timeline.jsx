...
export default function Timeline({ events }) {
  return (
    <>
      ...
      <div className="flex flex-col items-center mt-8">
        <ol className="relative border-l-2 border-green-400 dark:border-green-700">
          {events
            .sort((a, b) => {
              const dateA = new Date(a.date);
              const dateB = new Date(b.date);
            ...
            .map((event) => {
              ...
              return (
                <li key={event.name} className="mb-10 ms-4">
                  <div className="absolute w-3 h-3 bg-green-400 rounded-full mt-1.5 -start-1.5 border border-white dark:border-green-400 dark:bg-grren-400"></div>
                  ...
                  <div className="border border-gray-600 rounded-lg px-8 py-4 bg-gray-800 w-full text-center z-10 sm:w-96">
                    ...
                    <div className="text-gray-400 font-medium pb-6">
                      Registration deadline: {event.last_date_reg}
                    </div>
                    {new Date() < new Date(event.last_date_reg) && (
                      <Link
