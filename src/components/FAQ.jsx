    ...
    return (
        <div className="w-[80vw] flex flex-col items-center mx-auto px-4 md:px-20 font-mono min-h-[130vh]">
            <div className="flex flex-col md:flex-row items-center justify-center px-4 md:px-10 mb-10 w-full text-center md:text-left">
                ...
                <h1 className="text-2xl md:text-4xl font-bold text-[#fbeccd] [-webkit-text-stroke:2px_#726147] z-20 mx-4">Frequently Asked Questions</h1>
                <img src={faqImg} alt="" className="hidden md:block" />
            </div>
            <div className="space-y-4 w-full z-10 mb-4">
                {faqs.map((faq, index) => (
                    <div key={index}>
                        ...
                        >
                            <span className="text-black text-sm xl:text-lg font-bold pr-4">{faq.question}</span>
                            <svg
                        ...
                        {activeIndex === index && (
                            <div className="p-4 md:p-8 text-black bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#d6c999] via-[#bba87a] to-[#cab98a] opacity-100 rounded-[40px] border-solid border-[#8f774b] border-[5px]">{faq.answer}</div>
                        )}
                    </div>
                ))}
            </div>
