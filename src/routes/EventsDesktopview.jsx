                ...
                <div className="relative w-full max-w-7xl mx-auto px-4">
                    ...
                    <div className="flex flex-col gap-8 md:gap-0">
                        {sortedEvents.map((event, index) => {
                            ...
                            return (
                                <div key={event.id} className="relative flex flex-col md:flex-row items-center justify-between w-full mb-12">

                                    {/* Left Content */}
                                    <div className={`w-full md:w-[45%] flex justify-end ${isLeft ? '' : 'invisible'}`}>
                                    ...
                                    <div className={`hidden md:block absolute top-1/2 h-1 bg-yellow-500/50 w-[5%] ${isLeft ? 'left-[45%]' : 'right-[45%]'}`}></div>

                                    {/* Right Content */}
                                    <div className={`w-full md:w-[45%] flex justify-start ${isLeft ? 'invisible' : ''}`}>
                                        {!isLeft && <Timeline event={event} />}
                                    </div>
                                </div>
                            );
